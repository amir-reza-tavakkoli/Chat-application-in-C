...
void write_file(int sockfd,int sendid){
	...
	FILE *fp;
 	char filename[100] = "/home/foli/Desktop/server-recv";
  			if(blablabla==1){strcat(filename,"-third");}
  				if(blabla==1){strcat(filename,"-second");blablabla=1;}
    			if(bla==1){strcat(filename,"-first");blabla=1;}
  		
  			  strcat(filename,".txt");bla=1;
  int SIZE = 1024;
  char buffer[SIZE];
  fp = fopen(filename, "w");
  int i=0;
...
void * doNetworking(void * ClientDetail){
	...
	//char clientname[50] = clientDetail -> clientname;
	...
	while(1){
		...
		if(strcmp(data,"NAME")==0){//changing client name
		...
		if(strcmp(data,"HI")==0){int mc=0;//sending hi to everyone
			for(;mc<3;mc++){send(Client[mc].sockID,"hi everyone",1024,0);}}
			
			
			
		if(strcmp(data,"ME")==0){char s[200];sprintf(s,"You are number %d name %s\n",index+1,Client[index].clientname);send(clientSocket,s,1024,0);}
		
		
		if(strcmp(data,"S") == 0){//send message to specified client
		...
		if(strcmp(data,"M")==0){//send message to everyone
			...
			int mc=0;
			for(;mc<10;mc++){send(Client[mc].sockID,data,1024,0);}
		
		}
		
		//file input
		if(strcmp(data,"F")==0){read = recv(clientSocket,data,1024,0);
			data[read] = '\0';
...
if(strcmp(data,"jj")==0){if(backupcount>1){printf("errorr");exit(1);}strcpy(Client[index].clientname,"BACKUP SERVER");backupcount++;			int mc=0;
			for(;mc<10;mc++){send(Client[mc].sockID,"BACKUP SERVER ADDED",1024,0);}}
	//quiting a client
if(strcmp(data,"RU")==0){char ydata[45];sprintf(ydata,"%d disconnected\n",index+1);int mc=0;for(;mc<10;mc++){send(Client[mc].sockID,ydata,1024,0);}}

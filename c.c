...
void write_file(int sockfd,int sendid){// getting file from server
   int n;
   FILE *fp;
...
void write_files(int sockfd,int sendid){
 ...
 FILE *fp;
  char filename[40] = "recv";
  			...
  			//char filename[50]= "recv2.txt";
  			if(bla==1){strcat(filename,"m");blabla=1;}
  			if(blabla==1){strcat(filename,"m");}
...
void * doNetworking(void * ClientDetail){
	...
	while(1){
		...
		if(strcmp(data,"M")==0){
			...
			int mc=0;
			for(;mc<3;mc++){send(Client[mc].sockID,data,1024,0);}
...
void * doRecieving(void * sockID){// getting data
	...
	while(1){
		...
		char data[1024];
		int read = recv(clientSocket,data,1024,0);
		if(read<=0){close(clientSocket);mainn();}
...
int main(){
	...
	printf("in c\n");send(clientSocket,"jj",1024,0);
	
	pthread_t thread;
	pthread_create(&thread, NULL, doRecieving, (void *) &clientSocket );
printf("you cant send anything btw\n");
	while(1){
1+1;

...
void helperfunction(void){//a function for showing user how to interact with program
	printf("----------------------------------------------------------------\n"
"|                 Welcome to mini *chatbox*                    | \n"
"|           mainly designed for P.O.P course proj              | \n"
"|                          9732593                             | \n"
"|    Ive provided an elaborated documentation for this app     | \n"
...
void writenum(char *po){// writes the number of lines of file in lineof.txt
	FILE *fp; 
        int count = 0;char c;fp = fopen(po, "r");
...
int readnum(){  // reads number of lines from lineof.txt  
    FILE * fpt;
    char filenamee[100]="/home/foli/Desktop/lineof.txt";//DESKTOP PATH
...
void send_file(FILE *fp, int sockfd){// sending file to server
  int n;
  char data[SIZE];
// while there is sth
...
int backupserver(){
	...
	while(1){
		...
		if(strcmp(input,"M") == 0 ){
			...
			scanf("%[^\n]s",input);
			send(clientSocket,input,1024,0);/*printf("server m init\n");
					scanf("%[^\n]s",input);
			send(clientSocket,input,1024,0);
			*/
		...
		send(clientSocket,input,1024,0);
  		...
  		if(strcmp(input,"F")==0){
			...
			scanf("%s",input);
			send(clientSocket,input,1024,0);
			//send(clientSocket,"io",1024,0);
			scanf("%s",filename);
			
			 fp = fopen(filename, "r");	
			 writenum(filename);//count and write
			 if (fp == NULL) {
   			 printf("no");}
...
if(strcmp(input,"Q")==0){printf("quiting");exit(0);}
	}
	printf("finished");
	exit(1);
...
void * doRecieving(void * sockID){// getting data
	...
	while(1){
		...
		if(read<=0){close(clientSocket);delay(4);backupserver();exit(1);}
		...
		if(strcmp(data,"io")==0){
			...
			FILE *fp;
  			...
  			while (i<lineofcode) 
  			{
    			...
    			if(i>=lineofcode-1){
    				i=lineofcode+3;fclose(fp);break;
    			}i++;
...
int main(){
	...
	while(1){//infinite loop for clients
		...
		if(strcmp(input,"HI")==0){//saying hi
		send(clientSocket,input,1024,0);
		
		}	
		

  		if(strcmp(input,"F")==0){//file 
			send(clientSocket,input,1024,0);
			char filename[100];
			scanf("%s",input);
			send(clientSocket,input,1024,0);
			...
			scanf("%s",filename);
			//geting filename
			 fp = fopen(filename, "r");	//untill if thats vaild
			 	 while(fp == NULL){printf("no such file or directory, come again\n");scanf("%s",filename);fp = fopen(filename, "r");}
			 writenum(filename);//count and write
		...
		if(strcmp(input,"Q")==0){//exiting
		send(clientSocket,"RU",1024,0);printf("quiting,\n bye bye!");exit(0);
